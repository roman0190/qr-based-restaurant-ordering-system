...
import { Sparkles, Utensils, IndianRupee, Package, CheckCircle2 } from "lucide-react";

const categories = [
  "Appetizers", "Main Course", "Desserts", "Beverages", "Salads", 
  "Soups", "Pizza", "Burger", "Pasta", "Seafood", "Vegetarian", "Sides",
];
...
export default function ItemModal({
  ...
  async function submit() {
    try {
      ...
      const endpoint = method === "POST" ? "/api/items" : "/api/items/item";
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg rounded-[40px] border-none shadow-2xl bg-white/95 backdrop-blur-xl p-8 overflow-hidden">
        ...
        <div className="space-y-6 py-4 relative">
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent className="rounded-2xl border-none shadow-xl">
                  {categories.map((cat) => (
                    <SelectItem key={cat} value={cat} className="rounded-xl focus:bg-indigo-50 cursor-pointer">
                      {cat}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          {/* Image Upload Area */}
          <div className="space-y-2">
            <Label className="text-[11px] font-black uppercase tracking-widest text-gray-400 ml-1">Cover Photo</Label>
          ...
          <div className="flex items-center justify-between p-4 bg-gray-50/50 rounded-2xl border border-gray-100/50">
            <div className="flex items-center gap-3">
              <CheckCircle2 className={`h-5 w-5 ${form.available ? "text-green-500" : "text-gray-300"}`} />
              <div>
                <Label htmlFor="available" className="text-sm font-black text-gray-700 block leading-none">Available for sale</Label>
                <span className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">Show this in menu</span>
              </div>
            </div>
            <Checkbox
              id="available"
